...
IF EXISTS (SELECT NULL FROM [SYS].[OBJECTS] WITH (NOLOCK) WHERE [OBJECT_ID] = OBJECT_ID(N'[dbo].[sproc_SimplePlatForm_GetEventByID]') AND [type] IN (N'P', N'TF'))
DROP PROCEDURE [dbo].[sproc_SimplePlatForm_GetEventByID];
GO

CREATE PROCEDURE [dbo].[sproc_SimplePlatForm_GetEventByID]
(
	@ID		INT
)
AS
BEGIN
	...
	WITH [OfficeList] AS (
		...
		INNER JOIN dbo.[Events] AS [III] ON [III].EventId = @ID AND [III].[Office_OfficeId] = [II].[OfficeId]
	),
	[EventListWithCount] AS(
		...
		INNER JOIN [dbo].[Events] AS [I] ON I.IsDeleted = 0 AND [I].[EventId] = [III].[Event_EventId]
