...
IF EXISTS (SELECT NULL FROM [SYS].[OBJECTS] WITH (NOLOCK) WHERE [OBJECT_ID] = OBJECT_ID(N'[dbo].[sproc_SimplePlatForm_GetEventByOfficeIDs]') AND [type] IN (N'P', N'TF'))
DROP PROCEDURE [dbo].[sproc_SimplePlatForm_GetEventByOfficeIDs];
GO

CREATE PROCEDURE [dbo].[sproc_SimplePlatForm_GetEventByOfficeIDs]
(
...
BEGIN
	...
	[EventListWithCount] AS(
		...
		INNER JOIN [dbo].[Events] AS [I] ON I.IsDeleted = 0 AND I.StartDate BETWEEN @startDate AND @endDate AND [I].[EventId] = [III].[Event_EventId]
	...
	INNER JOIN [dbo].[Events] AS [I] ON I.IsDeleted = 0 AND I.StartDate BETWEEN @startDate AND @endDate AND [I].[convention_ConventionId] = [III].[ConventionId]
	INNER JOIN OfficeList AS [II] ON II.OfficeId = I.Office_OfficeId
