...
IF EXISTS (SELECT NULL FROM [SYS].[OBJECTS] WITH (NOLOCK) WHERE [OBJECT_ID] = OBJECT_ID(N'[dbo].[sproc_SimplePlatForm_GetFSMWeeklyCumulativeStatsByCurrentWeek]') AND [type] IN (N'P', N'TF'))
DROP PROCEDURE [dbo].[sproc_SimplePlatForm_GetFSMWeeklyCumulativeStatsByCurrentWeek];
GO

CREATE PROCEDURE [dbo].[sproc_SimplePlatForm_GetFSMWeeklyCumulativeStatsByCurrentWeek]
(
	@ID			INT
)
AS
BEGIN
	...
	WHERE [I].[IsDeleted] = 0 AND [I].[VisitType_VisitTypeId] IN (3, 4) AND [I].[FSMDetail_UserId] = @ID AND ([I].[VisitDate] BETWEEN @StartDate AND @EndDate);
