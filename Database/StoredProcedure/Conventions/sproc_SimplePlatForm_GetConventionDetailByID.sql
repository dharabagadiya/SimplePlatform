...
IF EXISTS (SELECT NULL FROM [SYS].[OBJECTS] WITH (NOLOCK) WHERE [OBJECT_ID] = OBJECT_ID(N'[dbo].[sproc_SimplePlatForm_GetConventionDetailByID]') AND [type] IN (N'P', N'TF'))
DROP PROCEDURE [dbo].[sproc_SimplePlatForm_GetConventionDetailByID];
GO

CREATE PROCEDURE [dbo].[sproc_SimplePlatForm_GetConventionDetailByID]
(
	@ConventionID	INT
)
AS
BEGIN
	...
	WITH [ConventonsList] AS (
		...
		WHERE IsDeleted = 0 AND I.ConventionId = @ConventionID
	),
