...
IF EXISTS (SELECT NULL FROM [SYS].[OBJECTS] WITH (NOLOCK) WHERE [OBJECT_ID] = OBJECT_ID(N'[dbo].[sproc_SimplePlatForm_GetActiveConventions]') AND [type] IN (N'P', N'TF'))
DROP PROCEDURE [dbo].[sproc_SimplePlatForm_GetActiveConventions];
GO

CREATE PROCEDURE [dbo].[sproc_SimplePlatForm_GetActiveConventions]
AS
BEGIN
	SET NOCOUNT ON;
