...
IF EXISTS (SELECT NULL FROM [SYS].[OBJECTS] WITH (NOLOCK) WHERE [OBJECT_ID] = OBJECT_ID(N'[dbo].[sproc_SimplePlatForm_GetTaskByID]') AND [type] IN (N'P', N'TF'))
DROP PROCEDURE [dbo].[sproc_SimplePlatForm_GetTaskByID];
GO

CREATE PROCEDURE [dbo].[sproc_SimplePlatForm_GetTaskByID]
(
	@ID		INT
)
AS
BEGIN
	...
	WHERE [I].[IsDeleted] = 0 AND [I].[TaskId] = @ID;
